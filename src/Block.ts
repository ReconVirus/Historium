...
export class TimelineProcessor {
    ...
    buildVerticalTimeline(timeline: HTMLElement, timelineNotes: Map<number, NoteData>, timelineDates: number[], settings: TimelinesSettings) {
        let eventCount = 0;
        for (let date of timelineDates) {
            let noteContainer = timeline.createDiv({cls: 'timeline-container'});
            let noteHeader = noteContainer.createEl('h2', {text: timelineNotes.get(date)[0].date.split('-').map((x, i) => (i === 0 && x === '0000') ? '0' : x.replace(/^0+/, '')).filter(x => x !== '00').join('-').replace(/-+/g, '-').replace(/-$/, ''), cls: 'timeline-header'});
            ...
            for (let eventAtDate of timelineNotes.get(date)) {
                ...
                let noteCard = eventContainer.createDiv({cls: `timeline-card ${eventAtDate.indicator}`});
                if (eventAtDate.image) {
                    noteCard.createDiv({cls: 'thumb', attr: {style: `background-image: url(${eventAtDate.image});`}});
                }
